...
// openmemory-js's library entry eagerly loads its dashboard HTTP server
// module, which prints [CONFIG]/[SERVER]/[DB]/[Vector]/[decay-2.0] lines to
// stdout/stderr at module-load and on every search/add call. In a
// long-running process like the DSH host that trashes any attached
// terminal. The clean fix is to run openmemory-js as its own systemd-managed
// HTTP service (see prompts/openmemory.service) and talk to it over the wire.
//
...
// primary_sector }. We pull only id/content/score. Failures of either call
// are non-fatal: recall returns [] and save silently does nothing. The
// plugin's recall and save paths already degrade gracefully when the memory
...
export const DEFAULT_MEMORY_URL =
  process.env.ERGON_MEMORY_URL ?? "http://127.0.0.1:8082";
  ...
  return {
    async recall(query: string, limit?: number): Promise<MemoryItem[]> {
      ...
      try {
        ...
        return items
          .filter(
            (m: any) =>
              m && typeof m === "object" && typeof m.content === "string" && m.content.length > 0,
          )
          .map((m: any) => ({
            id: String(m.id ?? ""),
    ...
    async save(content: string): Promise<void> {
      ...
      try {
        ...
        // Save errors are non-fatal — the steward fires post-turn and we
        // never want a memory write failure to surface to the user.
      }
    },
  };
}
