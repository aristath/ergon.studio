...
//
// The steward is a small LLM running in its own llama-server instance,
...
//      for the memory store. Returns null if the message has no searchable intent.
//
...
function loadDefinition(): StewardDefinition {
	...
	try {
		...
		throw new Error(
			`ergon steward: cannot read ${absPath}. ` +
...
export interface StewardRequestOptions {
	signal?: AbortSignal;
}
...
export const DEFAULT_MEMORY_DB_PATH: string = expandHomePath(
	String(
		process.env.ERGON_MEMORY_DB_PATH ??
			definition.config.memory_db_path ??
			"~/.local/share/ergon-memory-steward/memory.sqlite",
	),
);
export const DEFAULT_RECALL_LIMIT: number = numberConfig(
	process.env.ERGON_RECALL_LIMIT,
	definition.config.recall_limit,
	5,
);
...
export function createStewardClient(opts: StewardOptions = {}): StewardClient {
	...
	async function complete(
		...
		try {
			...
			// enable_thinking=false still emits empty tags; other models
			// may emit real reasoning we want to discard.
	...
	return {
		async rewriteQuery(
			userMessage: string,
			requestOpts: StewardRequestOptions = {},
		): Promise<string | null> {
		...
		async judgeSave(
			...
			const text = await complete(judgePrompt, exchange, requestOpts.signal);
			if (text === null) return null;
			try {
...
// ── Utility: content hash for dedup ──────────────────────────────────────────
