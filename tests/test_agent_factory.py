...
class AgentFactoryTests(unittest.TestCase):
    def test_build_agent_creates_openai_chat_agent_from_registry(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            agent = build_agent(
                ...
                model_id_override="qwen2.5-coder",
            )
    ...
    def test_build_agent_honors_request_model_override(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            agent = build_agent(
                registry, "orchestrator", model_id_override="gpt-oss-20b"
            )
    ...
    def test_build_agent_requires_request_model_override(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with self.assertRaisesRegex(
                ValueError, "proxy requests must supply a model"
            ):
    ...
    def test_build_agent_ignores_local_tool_metadata_without_registry(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            registry = _registry_with_agent(
                Path(temp_dir), metadata_extra={"tools": ["read_file"]}
            )

            agent = build_agent(
                registry, "orchestrator", model_id_override="qwen2.5-coder"
            )
...
def _registry_with_agent(
    home_dir: Path, *, metadata_extra: dict[str, object] | None = None
):
    ...
    frontmatter_lines.extend(
        ["---", "## Identity", "Lead engineer for the AI firm.", ""]
    )
