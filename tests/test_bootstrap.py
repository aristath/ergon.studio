...
class BootstrapProxyHomeTests(unittest.TestCase):
    def test_bootstrap_proxy_home_creates_global_layout(self) -> None:
    ...
    def test_bootstrap_proxy_home_does_not_overwrite_existing_definitions(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            home_dir = Path(temp_dir) / "home"
            home_dir.mkdir()
